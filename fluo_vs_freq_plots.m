...
if isempty(whos('fluo_data'))
    if dataset == 1121
        ...
        run /home/fperez/My_scan/November/21st/test_multiscan.m
    ...
    elseif dataset == 1127
        ...
        run /home/fperez/My_scan/November/27th/test_multiscan.m
...
title('live fluorescent signal (scan 1 only)')
ylabel('Fluorescent signal (AU)')
%legend(fieldnames(data))

subplot(3,1,3)
semilogy(times(:,1),d_sig./d_bg)
title('dead fluorescent signal (scan 1 only)')
xlabel('hours'), ylabel('Fluorescent signal (AU)')
%legend(fieldnames(data))
%%
