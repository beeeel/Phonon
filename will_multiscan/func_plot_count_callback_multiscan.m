...
dx = axis_info.axis2.um(2)-axis_info.axis2.um(1);
else
    dx=0;
end

if axis_info.axis3.pts>1
dy = axis_info.axis3.um(2)-axis_info.axis3.um(1);
else
    dy=0;
end
x_sel = find(0.5*dx+axis_info.axis2.um>x,1);
y_sel = find(0.5*dy+axis_info.axis3.um>y,1);
[ x x_sel y y_sel];
if x>max(axis_info.axis2.um)
    x_sel=axis_info.axis3.pts;
end
if y>max(axis_info.axis3.um)
    y_sel=axis_info.axis3.pts;
end
if x<min(axis_info.axis2.um)
    x_sel=1;
end
if y<min(axis_info.axis3.um)
    y_sel=1;
end
if isempty(x_sel)
    x_sel=axis_info.axis3.pts;
...
for k=1:length(figure_handles)
    ...
    if data.dc_count>0
        subplot(3,3,1);
        h2=get(gca,'Children');
        set(h2(1),'Xdata',axis_info.axis3.um(y_sel),'YData',axis_info.axis2.um(x_sel));
        %    h2(1).XData = axis_info.axis2.um(x_sel);
        %   h2(1).YData = axis_info.axis3.um(y_sel);
    end
    subplot(3,3,4);
    h2=get(gca,'Children');
    set(h2(1),'Xdata',axis_info.axis3.um(y_sel),'YData',axis_info.axis2.um(x_sel));
    ...
    xlabel('ns');title(sprintf('Raw trace:%1.1f,%1.2f',axis_info.axis2.um(x_sel),axis_info.axis3.um(y_sel)));
