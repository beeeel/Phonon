function func_plot_basic_1D_callback_multiscan(gcbo,eventdata,plotting_vars)
...
if isfield(axis_info,'axis1')
if axis_info.axis1.pts>1
...
for k=1:length(figure_handles)
    ...
    if data.dc_count>0
        subplot(3,3,1);
        h2=get(gca,'Children');
        set(h2(1),'Xdata',axis_info.axis1.um(x_sel),'YData',data.dc{k}(x_sel));
        %    h2(1).XData = axis_info.axis1.um(x_sel);
        %   h2(1).YData = axis_info.axis2.um(y_sel);
    end
    subplot(3,3,4);
    h2=get(gca,'Children');
    set(h2(1),'Xdata',axis_info.axis1.um(x_sel),'YData',data.f_amp{k}(x_sel));
    %h2(1).XData = axis_info.axis1.um(x_sel);
    %h2(1).YData = axis_info.axis2.um(y_sel);
    
    subplot(3,3,7);
    h2=get(gca,'Children');
    set(h2(1),'Xdata',axis_info.axis1.um(x_sel),'YData',data.freq{k}(x_sel));
    %h2(1).XData = axis_info.axis1.um(x_sel);
    %h2(1).YData = axis_info.axis2.um(y_sel);
    
    
    subplot(3,3,2:3);plot(1e9*data.raw_t{k},squeeze(data.raw_LP{k}(x_sel,:)));
    xlabel('ns');title(sprintf('Raw trace:%1.1f',axis_info.axis1.um(x_sel)));
