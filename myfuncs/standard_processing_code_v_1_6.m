...
exp_params.trace_length=800;            % how many points to use in trace for basic processing
exp_params.fit_order = 5;               % thermal removal fit order
exp_params.f_min = 5;                  % min freq of interest, used in freq search
exp_params.f_max = 6;                  % max freq of interest, used in freq search
exp_params.co_peak_range = (-50:100)+exp_params.default_loc;   %search range for co_peak
...
plot_params.plot_andor=1;                % For andor pictures, only in the Menlo lab plot before, after, freq and modep, good for cell stuff / Fernando
...
if fluo == 1
    fh = fluo_plot_fixup(fh, data, axis_info, exp_params, plot_params);
end
...
f_min =5.1                  %freq plot limit ranges
f_max = 5.9
