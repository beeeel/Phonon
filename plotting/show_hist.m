function [allFreq, fit, conf] = show_hist(ax, dir, fileBase, fitStartVal, fitLb, fitUb, fitOpts, exclude)
...
% Check start
if size(fitStartVal,2) ~= size(fitLb, 2) || size(fitStartVal,2) ~= size(fitUb, 2)
    fprintf('Size of start: %d \n Size of lb: %d \n Size of ub: %d \n', ...
        size(fitStartVal,2), size(fitLb,2), size(fitUb,2));
...
[fit, conf, pdf] = FitModel(allFreq, fitStartVal, fitLb, fitUb, fitOpts);
DrawHists(ax, allFreq, Npoints, Nbins, fitStartVal, pdf, fit, LineW, interpreter, fileBase);

    function [fit, conf, pdf] = FitModel(allFreq, start, lb, ub, options)
        % Define 3 models
        switch size(start,2)
            case 5
                % 2GMM (water, and cell)
    ...
    function DrawHists(ax, allFreq, Npoints, Nbins, start, pdf, fit, LineW, interpreter, fileBase)
        ...
        title(ax, fileBase,'Interpreter',interpreter)
    end
end
